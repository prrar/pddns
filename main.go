...
func getConfig(filePath string) (Config, error) {
	...
	if config.ZoneID == "" || config.Email == "" || config.APIToken == "" {
...
func getDNSRecords(zoneID, email, token string) ([]DNSRecord, error) {
...
func updateDNSRecord(zoneID, email, token, recordID string, update DNSUpdateRequest) error {
	...
	if res.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to update DNS record: %s", res.Status)
	}
...
func main() {
	...
	if *configFile == "" || *dnsEntry == "" {
		log.Fatal("Both --config and --dns-entry flags are required.")
	}
	...
	records, err := getDNSRecords(config.ZoneID, config.Email, config.APIToken)
	if err != nil {
	...
	if currentIP == targetRecord.IP {
		fmt.Println("IP address has not changed.")
		return
	}

	fmt.Printf("Updating DNS entry %s: IP changed from %s to %s.\n", *dnsEntry, targetRecord.IP, currentIP)
	...
	if err := updateDNSRecord(config.ZoneID, config.Email, config.APIToken, targetRecord.ID, update); err != nil {
